...
barnew <- ggplot(paired, aes(x=transect, y=newgrowth)) +
  geom_bar(stat="identity") + theme_classic() + theme(text = element_text(size=12))
...
fruits <- ggplot(paired, aes(x=fruits)) + geom_density() + theme(text = element_text(size=12))

reprodensities <- ggarrange(buds, fruits, ncol = 2, nrow = 1)
reprodensities
...
```{r, phys output viz, warning=FALSE}
#volume
...
vn <- ggplot(paired, aes(x=volume, y=newgrowth)) +
  geom_smooth(model = "glm", formula=y~x) + theme_classic() + theme(text = element_text(size=12))


#Volume and Branches
vb <- ggplot(paired, aes(x=volume, y=branches)) +
  geom_smooth(model = "glm", formula=y~x) + theme_classic() + theme(text = element_text(size=12))


#Branches and New growth
bn <- ggplot(paired, aes(x=branches, y=newgrowth)) +
  geom_smooth(model = "glm", formula=y~x) + theme_classic() + theme(text = element_text(size=12))

prefactors <- ggarrange(vn, vb, bn, ncol = 2, nrow = 2)
prefactors

```


####Absolute Size as Predictor Variables
...
##Models
###Test for multicolinearity and correlation between physical structure factors
...
#Correlation between reproductive output responses?
#buds and fruits
...
#For predictors as factors
vif(glm(buds~newgrowth + volume + branches, data=paired, family="quasipoisson"))
#doesn't look like it's a problem when they are just included in the model. w
#we can tell when a factor is a problem when it's VIF is much highest than the rest. 

#For predictors as interacting factors
vif(glm(buds~newgrowth * volume * branches, data=paired, family="quasipoisson"))
#some interactions are much too high, this model is not appropriate
...
m7<- glm(buds~fruits, data=paired, family="quasipoisson")
summary(m7)
